...
io$outdir <- file.path(io$basedir,"shiny/rna_expression/pseudobulk/stage_lineage"); dir.create(io$outdir, showWarnings = F)
...
cell_metadata.dt <- fread(io$metadata) %>%
  .[,stage_lineage:=sprintf("%s-%s",stage,lineage)]
...
pseudobulk_stats.dt <- cell_metadata.dt[,.N,by=c("stage","lineage","stage_lineage")]
...
# Update metadata
cell_metadata.dt <- cell_metadata.dt[stage_lineage%in%pseudobulk_stats.dt$stage_lineage]
...
sce$stage_lineage <- sprintf("%s-%s",sce$stage,sce$lineage)
...
to.save <- pseudobulk_stats.dt %>% copy %>% setnames("stage_lineage","id")
